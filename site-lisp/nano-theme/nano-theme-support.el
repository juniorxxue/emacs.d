...
;; transform json into a assoc list
(setq colors (json-read-file (expand-file-name "colors.json" (file-name-directory load-file-name))))

;; return red variables in colros by a number
(defun get-red (n)
  (let ((reds (cdr (assoc 'red colors))))
    (aref reds n)))

(defun get-green (n)
  (let ((greens (cdr (assoc 'green colors))))
    (aref greens n)))

(defun get-purple (n)
  (let ((purples (cdr (assoc 'purple colors))))
    (aref purples n)))

(defun get-orange (n)
  (let ((oranges (cdr (assoc 'orange colors))))
    (aref oranges n)))

(defun get-blue (n)
  (let ((blues (cdr (assoc 'blue colors))))
    (aref blues n)))
...
;; light
(defcustom nano-agda-keyword-light-foreground (get-red 6)
  "Agda keyword foreground color"
  :type 'color :group 'nano-theme-light)

(defcustom nano-agda-number-light-foreground (get-green 6)
  "Agda number foreground color"
  :type 'color :group 'nano-theme-light)

(defcustom nano-agda-module-light-foreground (get-purple 6)
  "Agda module foreground color"
  :type 'color :group 'nano-theme-light)

(defcustom nano-agda-inductive-constructor-light-foreground (get-green 6)
  "Agda inductive constructor color"
  :type 'color :group 'nano-theme-light)

(defcustom nano-agda-datatype-light-foreground (get-blue 6)
  "Agda datatype color"
  :type 'color :group 'nano-theme-light)

;; dark
(defcustom nano-agda-keyword-dark-foreground (get-red 3)
  "Agda keyword foreground color"
  :type 'color :group 'nano-theme-dark)

(defcustom nano-agda-number-dark-foreground (get-green 3)
  "Agda number foreground color"
  :type 'color :group 'nano-theme-dark)

(defcustom nano-agda-module-dark-foreground (get-purple 3)
  "Agda module foreground color"
  :type 'color :group 'nano-theme-dark)

(defcustom nano-agda-inductive-constructor-dark-foreground (get-green 3)
  "Agda inductive constructor color"
  :type 'color :group 'nano-theme-dark)

(defcustom nano-agda-datatype-dark-foreground (get-blue 3)
  "Agda datatype color"
  :type 'color :group 'nano-theme-dark)
...
(defface nano-agda-keyword nil
  "Agda keyword face"
  :group nil)

(defface nano-agda-module nil
  "Agda module face"
  :group nil)

(defface nano-agda-number nil
  "Agda number face"
  :group nil)

(defface nano-agda-inductive-constructor nil
  "Agda inductive constructor face"
  :group nil)

(defface nano-agda-datatype nil
  "Agda datatype face"
  :group nil)
...
(defun nano-theme (theme mode)
  ...
  (let ((light     '((background light)))
   ...
   ;; agda
   `(nano-agda-keyword ((,light (:foreground ,nano-agda-keyword-light-foreground))
                        (,dark (:foreground ,nano-agda-keyword-dark-foreground))))

   `(nano-agda-number ((,light (:foreground ,nano-agda-number-light-foreground))
                       (,dark (:foreground ,nano-agda-number-dark-foreground))))

   `(nano-agda-module ((,light (:foreground ,nano-agda-module-light-foreground))
                       (,dark (:foreground ,nano-agda-number-dark-foreground))))

   `(nano-agda-inductive-constructor ((,light (:foreground ,nano-agda-inductive-constructor-light-foreground))
                                      (,dark (:foreground ,nano-agda-inductive-constructor-dark-foreground))))

   `(nano-agda-datatype ((,light (:foreground ,nano-agda-datatype-light-foreground))
                         (,dark (:foreground ,nano-agda-datatype-dark-foreground))))
   ...
   '(font-lock-function-name-face  ((t (:inherit nano-salient))))
   ...
   ;; --- Agda -------------------------------------
   '(agda2-highlight-keyword-face   ((t (:inherit nano-agda-keyword))))
   '(agda2-highlight-module-face    ((t (:inherit nano-agda-module))))
   '(agda2-highlight-number-face    ((t (:inherit nano-agda-number))))
   '(agda2-highlight-function-face  ((t (:inherit nano-agda-module))))
   '(agda2-highlight-inductive-constructor-face ((t (:inherit nano-agda-inductive-constructor))))
   ...
   '(agda2-highlight-unsolved-meta-face ((t (:inherit highlight))))
   ...
   ;; -- Dashbord ------------------------------------------
   '(dashboard-items-face           ((t (:inherit nano-default))))
